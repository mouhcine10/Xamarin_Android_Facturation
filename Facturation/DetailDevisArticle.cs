...
{
    [Activity(Label = "Article", Theme = "@style/AppTheme.NoActionBar", ConfigurationChanges = ConfigChanges.Locale | ConfigChanges.ScreenSize | ConfigChanges.Orientation)]
    public class DetailDevisArticle : AppCompatActivity
    {
        ...
        private void UpdateItemDevis(string Article, float prix, int Qte, int TVA, string type, float remise, int iditemDevis)
        {
            ...
            contenu.CommandText = "Update commandeFacture Set Libille='" + Article + "', PrixHT=" + prix + ",Qte=" + Qte + ",TVA=" + TVA + ",Remise=" + remise + ",TypeProd='" + type + "' where Idcomfact=" + iditemDevis;
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            try
            {
                ...
                btnEnregister.Click += delegate
                {
                    ...
                    InsertionArticleItem(textArticle.Text, float.Parse(textprix.Text), int.Parse(textQuantite.Text), int.Parse(texttva.Text), toast, float.Parse(texttaux.Text), sharedPreferences.GetInt("IDDevis", 0));
        ...
        public override bool OnOptionsItemSelected(IMenuItem item)
        {
            ...
            {
                UpdateItemDevis(textArticle.Text, float.Parse(textprix.Text), int.Parse(textQuantite.Text), int.Parse(texttva.Text), toast, float.Parse(texttaux.Text), idItemDevis);
