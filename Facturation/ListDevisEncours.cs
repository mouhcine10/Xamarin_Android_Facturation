...
namespace Facturation
{
    ...
    public class ListDevisEncours : AppCompatActivity, NavigationView.IOnNavigationItemSelectedListener
    {
        static int i = 0;
        int IDD;
        ...
        private List<Devis> selectionlistDevis()
        {
            ...
            contenu.CommandText = "SELECT Devis.idDevis, DateValidation,NomClinet, Etat ,SUM((Prixht * Qte) + ((Prixht * Qte) * (TVA / 100)) - Remise),Devise from Devis INNER JOIN commandeDevis on Devis.idDevis = commandeDevis.IdDevis INNER JOIN   Client on Devis.idclient = Client.idclient WHERE Devis.Etat='Encours' GROUP by Devis.idDevis";
